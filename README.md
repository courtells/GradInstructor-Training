...
- **The editor is hidden from regular visitors.** By default, no one sees the "Edit content" button. To unlock it in your own browser, visit the site with `?edit` added to the URL once (e.g. `https://<your-username>.github.io/<repo-name>/?edit`) — it'll stay unlocked in that browser afterward. Visitors without that link never see any editing controls.
- **In the browser**: with the editor unlocked, click **"Edit content"** in the sidebar. Everything — module text, the sidebar title/subtitle, and every content block — can be added, rewritten, reordered, or deleted from there. Edits autosave to that browser's local storage only (they are not visible to other visitors until you publish them — see below).
- **Publish edits to everyone**: click **"Export code"** in the edit banner, copy the code shown, then on GitHub open `index.html` (pencil/edit icon), select and replace the existing `SITE_SETTINGS` object and `MODULES` array with the copied code, and commit the change. GitHub Pages will redeploy automatically within a minute or two.
...
- **`template.html`** is a blank starter with the same editor but no demo content — useful if you ever want to start a fresh copy of the site.
